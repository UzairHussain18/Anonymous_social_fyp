...
const validateSignup = [
  ...
  body('preferences').isArray({ min: 1 }).withMessage('Please select at least one preference')
];
...
router.post('/signup', validateSignup, async (req, res) => {
  try {
    ...
    const { email, phone, password, username, preferences, avatar } = req.body;
...
router.post('/login', validateLogin, async (req, res) => {
  try {
    ...
    // Update streak if user posted yesterday
